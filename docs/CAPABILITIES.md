...
| --- | --- | --- |
| Managed Agents MVP, separate policy | `src/server/managed/`, `/managed`, `/managed-runs/:id` | Durable Browserbase Agents API path with owner gates, immutable tasks, shared eight-agent ceiling, progress and agent-reported results. Default-off, exact approved initial origins; read-only/scope are prompts, not enforced confinement. No hard model/TTL limit, advanced workflows or native #8 acceptance. Hosted evidence must be recorded separately; offline tests are not that evidence |
