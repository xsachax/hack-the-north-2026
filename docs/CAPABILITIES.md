...
| --- | --- | --- |
| Authorized URL, scope and objective input | `src/lib/contracts.ts`, `public-execution.ts`, `target-scope.ts`; `src/server/target-policy.ts`, `api.ts` | `public-api.test.ts`, target/API/UI tests; explicit paired policy opt-in is separate from permanently blocked legacy website requests; public readiness remains disabled |
| Public HTTP(S) transport and factory, not enabled | `public-transport.ts`, `public-cloud.ts`, `public-network.ts`; shared `public-address.ts` | Offline DNS/socket/TLS proof plus actual CDP fixture mechanics; immutable scoped documents/separate public assets, bodyless GET/HEAD/OPTIONS, per-hop authorization and preserved response semantics. No hosted public-objective acceptance yet; [contract and remaining gates](EXECUTION.md#public-http-transport-library-offline-not-enabled) |
