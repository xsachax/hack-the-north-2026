...
| --- | --- | --- |
| Managed Agents MVP, separate policy | `src/server/managed/`, `/managed`, `/managed-runs/:id`, `src/components/managed-windows.tsx`, `GET /managed-runs/:id/sessions` | Durable Browserbase Agents API path with owner gates, immutable tasks, shared eight-agent ceiling, progress and agent-reported results. Default-off, exact approved initial origins; read-only/scope are prompts, not enforced confinement. No hard model/TTL limit, advanced workflows or native #8 acceptance. Live windows are pointer-blocked owner-only embeds served only while an attempt is active; hosted rendering unproved. Offline evidence: `src/server/managed/runner.test.ts`, `store.test.ts`, `api.test.ts` and the live-window cases in `tests/e2e/managed.spec.ts`. Hosted evidence must be recorded separately; offline tests are not that evidence |
| Authorized URL, scope and objective input | `src/lib/contracts.ts`, `public-execution.ts`, `target-scope.ts`; `src/server/target-policy.ts`, `api.ts` | `public-api.test.ts`, target/API/UI tests; explicit paired policy opt-in is separate from permanently blocked legacy website requests; operator enablement remains default-off |
| Public HTTP(S) transport and factory, hosted-validation candidate | `public-transport.ts`, `public-cloud.ts`, `public-network.ts`; shared `public-address.ts` | Offline DNS/socket/TLS proof plus actual CDP fixture mechanics; immutable scoped documents/separate public assets, bodyless GET/HEAD/OPTIONS, per-hop authorization and preserved response semantics. No hosted public-objective acceptance yet; [contract and remaining gates](EXECUTION.md#public-http-transport-library-offline-not-enabled) |
...
| Combined native startup and lifecycle | `composed-extension.ts`, `native-worker-control.ts`, `native-policy-session.ts`, `native-browser.ts`, `native-sdk.ts`, `native-sdk-worker.ts`, `native-resources.ts` | Exclusive SDK thread; real branded session/key attribution; parent-owned no-retry release; actual thread exit before metadata/CDP retirement. Actual WSS termination and composed Chromium/attestation/synthetic Gateway extraction regressions. Remote policy and genuinely refused proxy remain mandatory; no hosted acceptance yet |
| Guarded public-goal harness, not authorization | `scripts/public-proof.ts`, `public-goal.ts`, `public-integration.ts` | Existing-ledger-only offline plan; one-shot fresh approval bound to scope, code, packages and cumulative resource/usage inventory. Actual Browserbase/normal worker/API/wall/report path; source gate still enforced. Synthetic pipeline tests are not hosted acceptance; proof concurrency <=2 and shared lifetime reservations <=1,800s |
| Initial public action surface | Read-only driver plus unchanged criterion/citation engine | Scoped links/navigation/back/scroll/wait only; truthful observed controls, no buttons/forms/typing/keys/downloads/new tabs. Fresh profiles only; advanced controlled workflows remain gated |
| Trusted Gateway control path | Exact registered worker/offscreen identities, fixed endpoint, wire-budget guard | Actual owned-upstream body-hash/cookie-isolation and lifecycle tests; extra SDK dispatches spend retry budget, missing active authority fails closed; no page exception or provider-cookie forwarding |
| Native SDK trace export | Explicitly unsupported; reserved destination and local 403 denial | Exact POST from the verified service worker only; no trace forwarding, bounded private denial counter, no model charge. Real SDK flush and wrong-initiator/endpoint/limit regressions; native egress enforcement remains unchanged |
