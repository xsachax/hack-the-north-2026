...
| Combined native startup and lifecycle | `composed-extension.ts`, `native-worker-control.ts`, `native-policy-session.ts`, `native-browser.ts`, `native-resources.ts` | Local composed-byte/worker/profile/cleanup regressions; actual remote version and effective settings required, all URL overrides rejected, closed-proxy TCP evidence required, no policy removal before confirmed remote closure. Local evidence is not Browserbase admission proof |
